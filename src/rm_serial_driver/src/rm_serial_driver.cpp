      ...
      try {
          ...
          if (receiving_data) {
              ...
              if (header[0] == 0xAA) {
                  ...
                  // {
                  //   std::cout << "data_buffer: " << data_buffer.at(i) << std::endl;
                  // }
                  // 处理接收到的数据
                  //if (data_buffer.size() == sizeof(ReceivePacket) - 1) {
                      ...
                      // std::cout << "packet.reset_tracker" << packet.reset_tracker << std::endl;
                      // std::cout << "packet.is_shoot" << packet.is_shoot << std::endl;
                      // std::cout << "packet.pitch" << packet.pitch << std::endl;
                      // std::cout << "packet.yaw" << packet.yaw << std::endl;
                      // std::cout << "packet.roll" << packet.roll << std::endl;
                      serial_msg_.is_shoot = packet.is_shoot;
              ...
              receiving_data = true;
              data_buffer.push_back(header[0]);
              // RCLCPP_INFO(get_logger(), "start to receive");
          }
          else
          {
            // RCLCPP_INFO(get_logger(), "is not 0x5a!");
          }
      } catch (const std::exception & ex) {
          RCLCPP_ERROR_THROTTLE(
              get_logger(), *get_clock(), 20, "Error while receiving data: %s", ex.what());
...
void RMSerialDriver::sendData(const auto_aim_interfaces::msg::Serial msg)
{
