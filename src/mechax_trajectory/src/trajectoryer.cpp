...
Trajectoryer::Trajectoryer() : Node("trajectory")
{
    ...
    target_assistant_sub_ = this->create_subscription<auto_aim_interfaces::msg::Target>(
                  "/detector_assistant/target", rclcpp::SensorDataQoS(), std::bind(&Trajectoryer::target_assistant_callback, this, std::placeholders::_1));
...
//回调函数，用于接收tracker_assistant发布的消息
void Trajectoryer::target_assistant_callback(const auto_aim_interfaces::msg::Target msg)
{
    if(is_tracking == true)
    {
        return;
    }
    is_assistant_tracking = msg.tracking;
    float object_x = msg.position.x;
    float object_y = msg.position.y;
    float object_z = msg.position.z;
    two_resistance_model(object_x, object_y, object_z, v0, randa);
    angle_yaw = atan2(object_y, object_x);
    auto_aim_interfaces::msg::SendSerial result;
    result.header.frame_id = "odom";
    //--------------------------------------------
    ...
    result.is_can_hit = is_can_hit;
    if(std::isnan(send_pitch) || std::isnan(send_yaw))
    {
        result.is_tracking = false;
        result.is_can_hit = false;
        result.pitch = 0.0;
