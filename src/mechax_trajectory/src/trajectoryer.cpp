...
void Trajectoryer::get_need_yaw_pitch()
{
    float length = 0.01; //m

    float angle = atan2(ros_y, ros_x);
    float fix_x = ros_x - length * cos(angle);
    float fix_y = ros_y - length * sin(angle);

    fix_yaw = atan2(fix_y, fix_x);
    fix_pitch = two_resistance_model(fix_x, fix_y, ros_z, v0, randa);
}
