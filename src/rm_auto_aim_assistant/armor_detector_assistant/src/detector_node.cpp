...
namespace rm_auto_aim_assistant
{
ArmorDetectorNode::ArmorDetectorNode(const rclcpp::NodeOptions & options)
: Node("armor_detector_assistant", options)
{
  RCLCPP_INFO(this->get_logger(), "Starting DetectorAssistantNode!");

  // Detector
  detector_ = initDetector();

  target_pub_ = this->create_publisher<auto_aim_interfaces::msg::Target>(
    "/detector_assistant/target", rclcpp::SensorDataQoS());
  ...
  cam_info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
    ...
    "/camera_right/camera_info", rclcpp::SensorDataQoS(),
  ...
  img_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
    "/image_assistant", rclcpp::SensorDataQoS(),
...
void ArmorDetectorNode::status_callback(const auto_aim_interfaces::msg::Status::SharedPtr msg)
{
    ...
    {
        img_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
            "/image_assistant", rclcpp::SensorDataQoS(), std::bind(&ArmorDetectorNode::imageCallback, this, std::placeholders::_1));
...
void ArmorDetectorNode::imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr img_msg)
{
  ...
  if(diff.count() >= 1)
  {
    std::cout << diff.count() << "s and detector_assistant receive fps: " << detector_fps<< std::endl;
  ...
  if (pnp_solver_ != nullptr) {
    ...
    if(!armors_msg_.armors.empty()) {
      ...
      // 取识别到的armor的第一个作为tf的目标
      geometry_msgs::msg::PoseStamped ps;

      ps.header = armors_msg_.header;
      ps.pose = armors_msg_.armors[0].pose;
      try {
        ps.pose = tf2_buffer_->transform(ps, "odom").pose;
      ...
      target_msg.id = "assistant";
      target_msg.armors_num = 1;
      target_msg.position.x = ps.pose.position.x;
      target_msg.velocity.x = 0.0;
      target_msg.position.y = ps.pose.position.y;
      target_msg.velocity.y = 0.0;
      target_msg.position.z = ps.pose.position.z;
...
{
  lights_data_pub_ =
    this->create_publisher<auto_aim_interfaces::msg::DebugLights>("/detector_assistant/debug_lights", 10);
  armors_data_pub_ =
    this->create_publisher<auto_aim_interfaces::msg::DebugArmors>("/detector_assistant/debug_armors", 10);

  binary_img_pub_ = image_transport::create_publisher(this, "/detector_assistant/binary_img");
  number_img_pub_ = image_transport::create_publisher(this, "/detector_assistant/number_img");
  result_img_pub_ = image_transport::create_publisher(this, "/detector_assistant/result_img");
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(rm_auto_aim_assistant::ArmorDetectorNode)
