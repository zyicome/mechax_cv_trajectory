...
namespace back_mindvision_camera
{
class MVCameraNode : public rclcpp::Node
{
public:
  explicit MVCameraNode(const rclcpp::NodeOptions & options) : Node("back_mv_camera", options)
  {
    RCLCPP_INFO(this->get_logger(), "Starting BackMVCameraNode!");
    ...
    camera_pub_ = image_transport::create_camera_publisher(this, "image_back", qos);

    // Load camera info
    camera_name_ = this->declare_parameter("camera_name", "back_camera");
    ...
    capture_thread_ = std::thread{[this]() -> void {
      ...
      image_msg_.header.frame_id = "back_camera_optical_frame";
...
RCLCPP_COMPONENTS_REGISTER_NODE(back_mindvision_camera::MVCameraNode)
